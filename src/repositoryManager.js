    ...
    try {
      for (const repo of repositories) {
        ...
        results.push({
          ...
          Advanced_Security:
            repo.security_and_analysis?.advanced_security?.status || 'n/a',
          secret_scanning:
            repo.security_and_analysis?.secret_scanning?.status || 'n/a',
          ...
          secret_scanning_validity_checks:
            repo.security_and_analysis?.secret_scanning_validity_checks
              ?.status || 'n/a',
          hasActions,
          teams:
            teams.length > 0
